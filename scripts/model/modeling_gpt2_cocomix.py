...
class GPT2CoCoMixModel(GPT2PreTrainedModel):
    def __init__(self, config, concept_dim=0.1, insert_layer_idx=5, concept_num=32):
        ...
        self.pre_bias = nn.Parameter(torch.zeros(self.embed_dim))
        self.concept_predictor = nn.Linear(self.embed_dim, self.concept_dim, bias=True)
        self.compress_layer = nn.Linear(self.concept_dim, self.embed_dim, bias=True)
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if i == self.insert_layer_idx:
                # predicting concept
                bs, seq_len, hidden_dim = hidden_states.size()
                x, mu, std = LN(hidden_states)
                x = x - self.pre_bias
                ...
                stacked = torch.stack([hidden_states, continuous_concept], dim=2)
                hidden_states = stacked.view(bs, seq_len * 2, hidden_dim)

                # increase the attention mask as seq_len has increased
                if attention_mask is not None:
                    if self._attn_implementation == "flash_attention_2":
...
class GPT2CoCoMixLMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        get_concept_logit: Optional[bool] = None,
        ...
        if self.model_parallel:
            ...
            concept_logit = concept_logit.to(self.lm_head.weight.device)
